...
//for search page search for place holder
document.getElementById('place').value=`${obj.search}`;
    ...
    function displayVideos(data) {
      ...
      data.items.forEach((item) => {
        videoData = `
                      ...
                      <tr>
                      <td>
                      <a href="play.html?id=${item.id.videoId}">
                      <p style="color:white">${item.snippet.title}</p></a></td>
                      <td>
                      <a href="play.html?id=${item.id.videoId}">
                      <img width="400" height="200" src="${item.snippet.thumbnails.high.url}"/>
                      </td></a>
                      <!--<td>
